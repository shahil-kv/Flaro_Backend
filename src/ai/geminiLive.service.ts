...
export class GeminiLiveService {
  ...
  async startSession(systemPrompt: string, callbacks: GeminiSessionCallbacks) {
    ...
    logger.log(
      `[GeminiLiveService] [Session ${sessionId}] Starting new session.`,
    );
    ...
    try {
      ...
      const session = await this.ai.live.connect({
        ...
        callbacks: {
          onmessage: (msg: any) => {
            if (msg.data) {
              try {
                ...
                logger.error(
                  `[GeminiLiveService] [Session ${sessionId}] Error processing audio data:`,
                  error,
                );
              ...
              logger.log(
                `[GeminiLiveService] [Session ${sessionId}] Received turnComplete signal.`,
              );
              callbacks.onTurnComplete();
            } else {
              logger.warn(
                `[GeminiLiveService] [Session ${sessionId}] Unhandled message:`,
                JSON.stringify(msg),
              );
            }
          },
          onerror: (error: any) => {
            logger.error(
              `[GeminiLiveService] [Session ${sessionId}] Session error:`,
              error,
            );
          ...
          onclose: () => {
            logger.log(
              `[GeminiLiveService] [Session ${sessionId}] Session closed.`,
            );
            callbacks.onClose();
          },
        },
      ...
      logger.error(
        `[GeminiLiveService] [Session ${sessionId}] Failed to start session:`,
        error,
      );
      callbacks.onError(error as Error);
      return null;
