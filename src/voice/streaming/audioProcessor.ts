...
export class AudioProcessor {
  ...
  static async initialize() {
    ...
    try {
      ...
      this.srcDown = await create(1, 24000, 8000, {
        converterType: ConverterType.SRC_SINC_FASTEST,
      });
      this.srcUp = await create(1, 8000, 16000, {
        converterType: ConverterType.SRC_SINC_FASTEST,
      });
  ...
  static async processAudioForTwilio(inputBuffer: Buffer): Promise<Buffer> {
    if (!this.isInitialized || !this.srcDown) {
      logger.error(
        "[AudioProcessor] Down-sampler not initialized. Call initialize() first.",
      );
    ...
    const inputInt16 = new Int16Array(
      inputBuffer.buffer,
  ...
  static async processAudioForGemini(inputBuffer: Buffer): Promise<Buffer> {
    if (!this.isInitialized || !this.srcUp) {
      logger.error(
        "[AudioProcessor] Up-sampler not initialized. Call initialize() first.",
      );
      return Buffer.alloc(0);
    }
    if (!inputBuffer || inputBuffer.length === 0) {
